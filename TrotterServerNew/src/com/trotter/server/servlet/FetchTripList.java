...
public class FetchTripList extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
	    	...
	    	String[] mineTrip = (String[]) dbObject.get(MongoDBStructure.USER_TABLE_COLS.own_trips.name());
	    	
	    	if (mineTrip == null || mineTrip.length == 0) {
	    		response.setContentType("application/text");
	    	...
	    	for (String tripId : mineTrip) {
	    		BasicDBObject inTripQuery = new BasicDBObject();
	    		inTripQuery.put(MongoDBStructure.TRIP_TABLE_COLS._id.name(), new ObjectId(tripId));
	    		DBCursor tripCursor = tripTbl.find(inQuery);
